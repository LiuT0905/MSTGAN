...
# ---------------------------------------------------------------------------
# Fusion: PM + pressure concat, temperature/wind as global conditioning
# ---------------------------------------------------------------------------
class PMPressureTempWindFusion(nn.Module):
    def __init__(self,
                 pm_dim=512,
                 press_dim=512,
                 temp_dim=256,
        ...
        met_dim = temp_dim + wind_dim
        self.met_film = nn.Sequential(
            nn.Linear(met_dim, met_dim * 2),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(met_dim * 2, out_dim * 2)
        )
        self.met_gate = nn.Sequential(
            nn.Linear(met_dim, out_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self, pm_feat, press_feat, temp_nodes, wind_nodes, return_info=False):
        ...
        temp_global, temp_weight = self._weighted_pool(temp_nodes, self.temp_score)
        wind_global, wind_weight = self._weighted_pool(wind_nodes, self.wind_score)

        met_global = torch.cat([temp_global, wind_global], dim=1)   # [B, 512]

        gamma_beta = self.met_film(met_global)
        gamma, beta = torch.chunk(gamma_beta, 2, dim=1)
        gamma = gamma.unsqueeze(-1).unsqueeze(-1)                   # [B, 512, 1, 1]
        beta = beta.unsqueeze(-1).unsqueeze(-1)                     # [B, 512, 1, 1]

        met_gate = self.met_gate(met_global).unsqueeze(-1).unsqueeze(-1)
        ...
        # 让 PM 主干保留更多信息
        fused = pm_feat * (1.0 - met_gate) + x * met_gate
...
class MultiModalGenerator(nn.Module):
    def __init__(self, adj_norm, n_channels=12, n_classes=1):
        ...
        self.fusion = PMPressureTempWindFusion(
            pm_dim=512,
            press_dim=512,
            temp_dim=256,
            wind_dim=256,
    ...
    def forward(self, pm_seq, cp_layer, temp_data, wind_data, press_seq, return_info=False):
        ...
        if return_info:
            fused_feat, info = self.fusion(pm_feat, press_feat, temp_nodes, wind_nodes, return_info=True)
            return self.decoder(fused_feat, skips), info

        fused_feat = self.fusion(pm_feat, press_feat, temp_nodes, wind_nodes)
...
def main():
    ...
    parser.add_argument('--temp_path', default=r"D:\dataset\TEMP_daily")
    parser.add_argument('--wind_path', default=r"D:\dataset\WDSP_daily")
    ...
    parser.add_argument('--outf', default=r'D:\MMSTGAN\experiment\4')
    ...
    optimizerD = torch.optim.Adam(netD.parameters(), lr=opt.lr * 0.2, betas=(0.5, 0.999))
