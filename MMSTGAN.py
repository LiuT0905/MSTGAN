...
class PMEncoder(nn.Module):
    def __init__(self, in_channels=12, sequence_length=11,
        ...
        hidden_dim = 32
        self.temporal_conv = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=hidden_dim, kernel_size=(3, 1), padding=(1, 0), bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.GELU(),
            nn.Conv2d(in_channels=hidden_dim, out_channels=1, kernel_size=(1, 1), bias=False),
            nn.BatchNorm2d(1)
        )
    ...
    def forward(self, pm_seq, cp_layer, dem_feats=None, met_global=None):
        x = torch.cat((pm_seq, cp_layer), dim=1)
        original_x = x  # [B, 12, H, W]
        B, T, H, W = x.shape
        x_t = x.reshape(B, 1, T, H * W)  # [B, 1, 12, H*W]
        x_res = self.temporal_conv(x_t)  # [B, 1, 12, H*W]
        x_res = x_res.reshape(B, T, H, W)  # [B, 12, H, W]
        x = original_x + x_res
...
class MultiModalGenerator(nn.Module):
    ...
    def forward(self, pm_seq, cp_layer, temp_data, wind_data, press_seq, dem_img, return_info=False):
        ...
        met_global = self.met_conditioner(temp_nodes, wind_nodes)  # [B, 512]
...
def main():
    ...
    parser.add_argument('--outf', default=r'autodl-tmp/experiment/t5')
