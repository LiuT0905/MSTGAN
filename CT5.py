...
class MultiModalDataset(Dataset):
    def __init__(self, pm_folder, temp_folder, wind_folder, press_folder, dem_path,
        ...
        print("Computing dataset statistics for normalization (temp / wind / press / dem)...")
        ...
        self.dem_img = torch.from_numpy(dem_img).unsqueeze(0).float()  # [1, 128, 128]
    ...
    def __getitem__(self, idx):
        ...
        dem_img = self.dem_img.clone()  # [1, 128, 128]

        return pm_seq, press_seq, dem_img, masked_last_pm, temp_values, wind_values, last_pm_real
...
class PressureViT1DEncoder(nn.Module):
    def __init__(self, image_size=64, num_frames=4, patch_size=8,
...
class PMEncoder(nn.Module):
    def __init__(self,
        ...
        # ==========================================================
        # DEM：Generic Concatenation Fusion
        # ==========================================================
        self.dem_fuse1 = ProjectedConcatFusion2D(64, 64)
        self.dem_fuse2 = ProjectedConcatFusion2D(128, 128)
        self.dem_fuse3 = ProjectedConcatFusion2D(256, 256)
        self.dem_fuse4 = ProjectedConcatFusion2D(512, 512)

        # ==========================================================
        # Temperature/Wind：Generic Concatenation Fusion
        # ==========================================================
        self.met_fuse1 = ProjectedGlobalConcatFusion(512, 64)
        self.met_fuse2 = ProjectedGlobalConcatFusion(512, 128)
        self.met_fuse3 = ProjectedGlobalConcatFusion(512, 256)
        self.met_fuse4 = ProjectedGlobalConcatFusion(512, 512)
    ...
    def forward(self,
        ...
        # ----------------------------------------------------------
        # PM + target-time sparse control-point field
        # ----------------------------------------------------------
        x = torch.cat(
            (pm_seq, cp_layer),
            dim=1
        ...
        # ----------------------------------------------------------
        # Temperature / Wind generic concatenation
        # ----------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Generic Projected Concatenation Fusion
# 用于融合二维空间特征：PM + DEM / PM + Pressure
# ---------------------------------------------------------------------------
class ProjectedConcatFusion2D(nn.Module):
    """
    通用二维特征拼接融合：
    ...
    因此适合作为 proposed modality-specific fusion 的
    ...
    def __init__(self,
        ...
        # 与原模型类似保留基础特征细化能力，
        # 避免 simple baseline 因容量过弱而不公平
        self.refine = ResidualConvBlock(
            pm_channels,
            dropout=dropout
        )

    def forward(self, pm_feat, aux_feat):
        # 空间尺寸对齐
        if aux_feat.shape[-2:] != pm_feat.shape[-2:]:
...
# ---------------------------------------------------------------------------
# Generic Global Concatenation Fusion
# 用于融合 PM 特征 + Temp/Wind 全局向量
# ---------------------------------------------------------------------------
class ProjectedGlobalConcatFusion(nn.Module):
    """
    将 Temp/Wind 得到的 global vector：
    ...
    与 PM feature map 直接拼接。

    用于替代原来的 Global FiLM Modulation。
    """
    ...
    def forward(self, pm_feat, cond):
        B, C, H, W = pm_feat.shape
...
class MultiModalGenerator(nn.Module):
    def __init__(self,
        ...
        # PM
        # ----------------------------------------------------------
        self.pm_encoder = PMEncoder(
            in_channels=pm_seq_len + 1,
            sequence_length=pm_seq_len
        )

        # ----------------------------------------------------------
        # Temperature / Wind
        # ----------------------------------------------------------
        self.temp_encoder = TempGCNEncoder(
            input_dim=1,
            hidden_dim=128,
            output_dim=256
        )

        self.wind_encoder = WindGCNEncoder(
            input_dim=1,
            hidden_dim=128,
            output_dim=256
        )

        self.met_conditioner = TempWindConditioner(
            temp_dim=256,
            wind_dim=256
        )
        ...
        # 修改：
        # 原 PMPressureFusion
        # -> Generic Projected Concatenation
        # ----------------------------------------------------------
        self.press_fusion = ProjectedConcatFusion2D(
            pm_channels=512,
            aux_channels=512,
            dropout=0.1
        )
    ...
    def forward(self,
        ...
        # DEM encoder
        dem_feats = self.dem_encoder(
            dem_img
        )

        # Temp / Wind encoder
        temp_nodes = self.temp_encoder(
            temp_data,
            self.adj_norm
        )

        wind_nodes = self.wind_encoder(
            wind_data,
            self.adj_norm
        )

        met_global = self.met_conditioner(
            temp_nodes,
            wind_nodes
        )
        ...
        # Pressure
        press_feat = self.press_encoder(
            press_seq
        )

        # Generic concatenation
        fused_feat = self.press_fusion(
            pm_feat,
            press_feat
        )
...
def main():
    ...
    parser.add_argument('--dem_path', default=r"autodl-tmp/dataset/dem/dem_128.png")
    parser.add_argument('--mask_path', default=r"mask/mask_huabei.txt")

    parser.add_argument('--outf', default=r'autodl-tmp/experiment/Ct5')
    ...
    optimizerD = torch.optim.Adam(netD.parameters(), lr=opt.lr * 0.3, betas=(0.5, 0.999))
    ...
    for epoch in range(1, opt.niter + 1):
        ...
        for i, (imgs, press_imgs, dem_img, masked_last, temp_val, wind_val, last_pm_real) in enumerate(dataloader):
            ...
            dem_img = dem_img.to(device)  # [B, 1, 128, 128]
            ...
            fake_pm = netG(input_seq, cp_layer, temp_val, wind_val, press_imgs, dem_img)
            out_fake = netD(fake_pm.detach(), cp_layer)
            loss_d_fake = criterion_gan(out_fake, torch.full_like(out_fake, 0.1))
