...
class TemperatureConcatFusion(nn.Module):
    def __init__(self, pm_dim=512, temp_dim=512, out_dim=512, dropout=0.1):
        super().__init__()
        self.fuse_in = nn.Sequential(
            nn.Conv2d(pm_dim + temp_dim, out_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_dim),
            nn.ReLU(inplace=True)
        )

        self.res_block1 = ResidualConvBlock(out_dim, dropout=dropout)
        # self.res_block2 = ResidualConvBlock(out_dim, dropout=dropout)
    ...
    def forward(self, pm_feat, temp_feat):
        """
        pm_feat   : [B, 512, H, W]
        temp_feat : [B, 512, h, w]
        """
        if temp_feat.shape[-2:] != pm_feat.shape[-2:]:
        ...
        x = torch.cat([pm_feat, temp_feat], dim=1)  # [B, 1024, H, W]
        x = self.fuse_in(x)                         # [B, 512, H, W]
        x = self.res_block1(x)                      # [B, 512, H, W]
        # x = self.res_block2(x)                      # [B, 512, H, W]
...
class MultiModalGenerator(nn.Module):
    def __init__(self, n_channels=12, n_classes=1):
        ...
        self.fusion = TemperatureConcatFusion(
            pm_dim=512, temp_dim=512, out_dim=512, dropout=0.1
        )
...
def main():
    ...
    for epoch in range(1, opt.niter + 1):
        ...
        for i, (imgs, press_imgs, masked_last, last_pm_real) in enumerate(dataloader):
            ...
            fake_pm = netG(input_seq, cp_layer, press_imgs)
