...
class MultiModalGenerator(nn.Module):
    ...
    def forward(self, pm_seq, cp_layer, temp_data, wind_data, press_seq):
...
def main():
    ...
    for epoch in range(1, opt.niter + 1):
        ...
        for i, (imgs, press_imgs, masked_last, temp_val, wind_val, last_pm_real) in enumerate(dataloader):
            ...
            input_seq = imgs[:, :-1].squeeze(2).to(device)
            cp_layer = masked_last.squeeze(2).to(device)
            temp_val = temp_val.to(device)
            wind_val = wind_val.to(device)
            press_imgs = press_imgs.to(device)
